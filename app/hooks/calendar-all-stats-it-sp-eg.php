<?php
	...
	<script>
		$j(function() {
			...
			Cal._fullCal = new FullCalendar.Calendar($j('#' + calId).get(0), {
				...
				eventSources: [
					{
						url: 'calendar-events-italy-all-stats.json.php',
						failure: function() {
							$j('#' + calId + '-events-loading-error').removeClass('hidden');
						}
					},
					{
						url: 'calendar-events-spain-all-stats.json.php',
						failure: function() {
							$j('#' + calId + '-events-loading-error').removeClass('hidden');
						}
					},
					{
						url: 'calendar-events-egypt-all-stats.json.php',
						failure: function() {
							$j('#' + calId + '-events-loading-error').removeClass('hidden');
						}
					},
				...
				eventClick: function(e) {
					...
					if(e.event.url) {
						...
						var title = $j(e.event.title).text(), maxChars = 100;
				...
				datesRender: function(i) {
					...
					switch(i.view.type) {
						...
						case 'listWeek':
							setTimeout(function() {
								Cal._fullCal.setOption('height', 'auto');
								Cal._fullCal.setOption('contentHeight', 'auto');
							}, 5);
							break;
						case 'timeGridWeek':
						case 'timeGridDay':
							setTimeout(function() {
								Cal._fullCal.setOption('height', undefined);
								Cal._fullCal.setOption('contentHeight', undefined);
								Cal._fullCal.scrollToTime(Cal.scrollTime);
							}, 5);
							break;
					}
				},
			...
			Cal.Translate.ready(function() {
				Cal.newEventButtons.create([
					{
						type: 'italy-all-stats',
						color: 'danger',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'italy all stats' }),
						table: 'confirmed'
					},
					{
						type: 'spain-all-stats',
						color: 'warning',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'spain all stats' }),
						table: 'confirmed'
					},
					{
						type: 'egypt-all-stats',
						color: 'primary',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'egypt all stats' }),
						table: 'confirmed'
					},
				]);
			});
	...
	<div class="page-header"><h1>
		...
		All stats (Italy, Spain, Egypt)				</h1></div>

	<div 
		id="all-stats-it-sp-eg-loading" 
	...
	<div id="all-stats-it-sp-eg-events-loading-error" class="hidden alert alert-warning text-center">
