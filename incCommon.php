	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = array(
			...
			'confirmed' => array('Confirmed COVID-19 cases', 'Data for confirmed, recovered and death cases, as imported from <a href="https://data.humdata.org/dataset/novel-coronavirus-2019-ncov-cases">humdata.org</a><br><div class="text-danger"><br>When importing the CSV, please make sure NOT to import to the fields <code>calc_date</code>, <code>calc_recovered</code> or <code>calc_deaths</code> to avoid corrupted data!<br></div>', 'resources/table_icons/chart_bar_error.png', 'None'),
			'recovered' => array('Recovered', '', 'resources/table_icons/health.png', 'None'),
			'deaths' => array('Deaths', '', 'resources/table_icons/color_gradient.png', 'None')
		);
	...
	function get_sql_fields($table_name) {
		$sql_fields = array(
			'confirmed' => "`confirmed`.`id` as 'id', `confirmed`.`country` as 'country', `confirmed`.`state` as 'state', `confirmed`.`lat` as 'lat', `confirmed`.`long` as 'long', if(`confirmed`.`date`,date_format(`confirmed`.`date`,'%d/%m/%Y'),'') as 'date', `confirmed`.`value` as 'value', `confirmed`.`calc_recovered` as 'calc_recovered', `confirmed`.`calc_deaths` as 'calc_deaths'",
			'recovered' => "`recovered`.`id` as 'id', `recovered`.`state` as 'state', `recovered`.`country` as 'country', `recovered`.`lat` as 'lat', `recovered`.`long` as 'long', if(`recovered`.`date`,date_format(`recovered`.`date`,'%d/%m/%Y'),'') as 'date', `recovered`.`value` as 'value'",
			'deaths' => "`deaths`.`id` as 'id', `deaths`.`state` as 'state', `deaths`.`country` as 'country', `deaths`.`lat` as 'lat', `deaths`.`long` as 'long', if(`deaths`.`date`,date_format(`deaths`.`date`,'%d/%m/%Y'),'') as 'date', `deaths`.`value` as 'value'",
		);
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> COVID-19 stats</a>
			</div>
	...
	function NavMenus($options = array()) {
		...
		if(is_array($arrTables)) {
			foreach($arrTables as $tn => $tc) {
				...
				$tChkHL = array_search($tn, array('recovered','deaths'));
