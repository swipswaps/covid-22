		...
		$all_tables = array(
			/* ['table_name' => [table props assoc array] */   
				'confirmed' => array(
					...
					'Description' => 'Data for confirmed, recovered and death cases, as imported from <a href="https://data.humdata.org/dataset/novel-coronavirus-2019-ncov-cases">humdata.org</a><br><div class="text-danger"><br>When importing the CSV, please make sure NOT to import to the fields <code>calc_date</code>, <code>calc_recovered</code> or <code>calc_deaths</code> to avoid corrupted data!<br></div>',
					'tableIcon' => 'resources/table_icons/chart_bar_error.png',
					'group' => $tg[0],
					'homepageShowCount' => 0
				),
				'recovered' => array(
				...
				'deaths' => array(
					'Caption' => 'Deaths',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/color_gradient.png',
					'group' => $tg[0],
					'homepageShowCount' => 0
				),
		);
...
########################################################################
	...
	*/
	...
	function calculated_fields() {
		...
		return array(
			'confirmed' => array(
				'calc_recovered' => 'SELECT `recovered`.`value` FROM `confirmed`
LEFT JOIN `recovered` ON 
	`recovered`.`state` <=> `confirmed`.`state` AND
	`recovered`.`country` <=> `confirmed`.`country` AND
	`recovered`.`date` = `confirmed`.`date` AND
	`recovered`.`lat` <=> `confirmed`.`lat` AND
	`recovered`.`long` <=> `confirmed`.`long`
WHERE `confirmed`.`id`=\'%ID%\'',
				'calc_deaths' => 'SELECT `deaths`.`value` FROM `confirmed`
LEFT JOIN `deaths` ON 
	`deaths`.`state` <=> `confirmed`.`state` AND
	`deaths`.`country` <=> `confirmed`.`country` AND
	`deaths`.`date` = `confirmed`.`date` AND
	`deaths`.`lat` <=> `confirmed`.`lat` AND
